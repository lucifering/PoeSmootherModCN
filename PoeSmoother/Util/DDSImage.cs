...
namespace KUtility {
	public class DDSImage {
        ...
        private const int FOURCC_DXT2 = 0x32545844;
			...
			using (MemoryStream ms = new MemoryStream(rawdata)) {
				using (BinaryReader r = new BinaryReader(ms)) {
					...
					if (dwMagic != 0x20534444) {
                        ...
                        {
                            throw new Exception(Settings.Strings["Error_DDS_Unsupported"] + System.Text.Encoding.Default.GetString(rawdata));
                        }
                        else {
                            throw new Exception(Settings.Strings["Error_DDS_Unsupported_NUM"] + dwMagic.ToString("x"));
                        }
		...
		private Bitmap readBlockImage(byte[] data, int w, int h) {
			switch (header.ddspf.dwFourCC) {
                ...
                case FOURCC_DXT3:
                    ...
                    return UncompressDXT5(data, w, h);
                   // throw new Exception(string.Format("0x{0}  DXT3格式的DDS 暂时不支持预览，导出后用其他软件查看-光影.", header.ddspf.dwFourCC.ToString("X")));
                case FOURCC_DXT2:
                    //throw new Exception(string.Format("0x{0}  DXT2格式的DDS 暂时不支持预览，导出后用其他软件查看-光影.", header.ddspf.dwFourCC.ToString("X")));
